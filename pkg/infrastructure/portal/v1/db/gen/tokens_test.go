...
var (
	tokenDBTypes = map[string]string{`ID`: `varchar`, `OwnerID`: `varchar`, `IssuerID`: `varchar`, `Active`: `tinyint`, `ExpiresAt`: `timestamp`, `CreatedAt`: `timestamp`, `UpdatedAt`: `timestamp`}
	_            = bytes.MinRead
)
...
func testTokensUpsert(t *testing.T) {
	...
	if len(mySQLTokenUniqueColumns) == 0 {
