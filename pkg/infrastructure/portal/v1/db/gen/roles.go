...
var (
	...
	roleColumnsWithoutDefault = []string{"id", "name", "description", "created_at", "updated_at"}
...
func (o *Role) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to populate default values for roles")
	}
...
func (o *Role) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert for roles")
	}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to retrieve unique values for roles")
	}
...
var (
	RoleAllColumns            = roleAllColumns
	RoleColumnsWithoutDefault = roleColumnsWithoutDefault
	RoleColumnsWithDefault    = roleColumnsWithDefault
	RolePrimaryKeyColumns     = rolePrimaryKeyColumns
	RoleGeneratedColumns      = roleGeneratedColumns
)
...
func (s RoleSlice) GetIDs() []string {
...
func (s RoleSlice) GetIntfIDs() []interface{} {
...
func (s RoleSlice) ToIDMap() map[string]*Role {
	result := make(map[string]*Role, len(s))
	for _, o := range s {
		result[o.ID] = o
	}
	return result
...
func (s RoleSlice) ToUniqueItems() RoleSlice {
	result := make(RoleSlice, 0, len(s))
...
func (s RoleSlice) FindItemByID(id string) *Role {
	for _, o := range s {
		if o.ID == id {
			return o
		}
...
func (s RoleSlice) FindMissingItemIDs(expectedIDs []string) []string {
	if len(s) == 0 {
		return expectedIDs
	}
	result := []string{}
...
func (o RoleSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		wl, _ := columns.InsertColumnSet(
			...
			roleColumnsWithDefault,
			roleColumnsWithoutDefault,
			queries.NonZeroDefaultSet(roleColumnsWithDefault, row),
		)
		for _, col := range wl {
			wlCols[col] = struct{}{}
		}
	...
	for _, col := range roleAllColumns {
	...
	for i, row := range o {
		...
		if i == 0 {
			sql = "INSERT INTO `roles` " + "(`" + strings.Join(wl, "`,`") + "`)" + " VALUES "
		}
		sql += strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), len(vals)+1, len(wl))
		if i != len(o)-1 {
			sql += ","
		}
		valMapping, err := queries.BindMapping(roleType, roleMapping, wl)
		if err != nil {
			return 0, err
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to insert all from role slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by insertall for roles")
	}

	if len(roleAfterInsertHooks) != 0 {
		for _, obj := range o {
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o RoleSlice) InsertIgnoreAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o RoleSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		nzUniques := queries.NonZeroDefaultSet(mySQLRoleUniqueColumns, row)
		if len(nzUniques) == 0 {
	...
	if !updateColumns.IsNone() && len(update) == 0 {
		return 0, errors.New("models: unable to upsert roles, could not build update column list")
	}
	...
	if len(update) == 0 {
		fmt.Fprintf(
			buf,
			"INSERT IGNORE INTO `roles`(%s) VALUES %s",
		...
		fmt.Fprintf(
			buf,
			"INSERT INTO `roles`(%s) VALUES %s ON DUPLICATE KEY UPDATE ",
			strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, insert), ","),
			strmangle.Placeholders(false, len(insert)*len(o), 1, len(insert)),
		)
	...
	query := buf.String()
	valueMapping, err := queries.BindMapping(roleType, roleMapping, insert)
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to upsert for roles")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by upsert for roles")
	}

	if len(roleAfterUpsertHooks) != 0 {
		for _, obj := range o {
...
// DeleteAllByPage delete all Role records from the slice.
...
func (s RoleSlice) DeleteAllByPage(ctx context.Context, exec boil.ContextExecutor, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpdateAllByPage update all Role records from the slice.
// This function updates data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s RoleSlice) UpdateAllByPage(ctx context.Context, exec boil.ContextExecutor, cols M, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// InsertAllByPage insert all Role records from the slice.
// This function inserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s RoleSlice) InsertAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
	...
	// MySQL max placeholders = 65535
	chunkSize := MaxPageSize / reflect.ValueOf(&RoleColumns).Elem().NumField()
...
// InsertIgnoreAllByPage insert all Role records from the slice.
// This function inserts data by pages to avoid exceeding Postgres limitation (max parameters: 65535)
func (s RoleSlice) InsertIgnoreAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpsertAllByPage upsert all Role records from the slice.
// This function upserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s RoleSlice) UpsertAllByPage(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
