...
var (
	InvitationAllColumns            = invitationAllColumns
	InvitationColumnsWithoutDefault = invitationColumnsWithoutDefault
	InvitationColumnsWithDefault    = invitationColumnsWithDefault
	InvitationPrimaryKeyColumns     = invitationPrimaryKeyColumns
	InvitationGeneratedColumns      = invitationGeneratedColumns
)
...
func (s InvitationSlice) GetIDs() []string {
...
func (s InvitationSlice) GetIntfIDs() []interface{} {
...
func (s InvitationSlice) ToIDMap() map[string]*Invitation {
	result := make(map[string]*Invitation, len(s))
	for _, o := range s {
		result[o.ID] = o
	}
	return result
...
func (s InvitationSlice) ToUniqueItems() InvitationSlice {
	result := make(InvitationSlice, 0, len(s))
...
func (s InvitationSlice) FindItemByID(id string) *Invitation {
	for _, o := range s {
		if o.ID == id {
			return o
		}
...
func (s InvitationSlice) FindMissingItemIDs(expectedIDs []string) []string {
	if len(s) == 0 {
		return expectedIDs
	}
	result := []string{}
...
func (o InvitationSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		wl, _ := columns.InsertColumnSet(
			...
			invitationColumnsWithDefault,
			invitationColumnsWithoutDefault,
			queries.NonZeroDefaultSet(invitationColumnsWithDefault, row),
		)
		for _, col := range wl {
			wlCols[col] = struct{}{}
		}
	...
	for _, col := range invitationAllColumns {
	...
	for i, row := range o {
		...
		if i == 0 {
			sql = "INSERT INTO `invitations` " + "(`" + strings.Join(wl, "`,`") + "`)" + " VALUES "
		}
		sql += strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), len(vals)+1, len(wl))
		if i != len(o)-1 {
			sql += ","
		}
		valMapping, err := queries.BindMapping(invitationType, invitationMapping, wl)
		if err != nil {
			return 0, err
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to insert all from invitation slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by insertall for invitations")
	}

	if len(invitationAfterInsertHooks) != 0 {
		for _, obj := range o {
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o InvitationSlice) InsertIgnoreAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o InvitationSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		nzUniques := queries.NonZeroDefaultSet(mySQLInvitationUniqueColumns, row)
		if len(nzUniques) == 0 {
	...
	if !updateColumns.IsNone() && len(update) == 0 {
		return 0, errors.New("models: unable to upsert invitations, could not build update column list")
	}
	...
	if len(update) == 0 {
		fmt.Fprintf(
			buf,
			"INSERT IGNORE INTO `invitations`(%s) VALUES %s",
		...
		fmt.Fprintf(
			buf,
			"INSERT INTO `invitations`(%s) VALUES %s ON DUPLICATE KEY UPDATE ",
			strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, insert), ","),
			strmangle.Placeholders(false, len(insert)*len(o), 1, len(insert)),
		)
	...
	query := buf.String()
	valueMapping, err := queries.BindMapping(invitationType, invitationMapping, insert)
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to upsert for invitations")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by upsert for invitations")
	}

	if len(invitationAfterUpsertHooks) != 0 {
		for _, obj := range o {
...
// DeleteAllByPage delete all Invitation records from the slice.
...
func (s InvitationSlice) DeleteAllByPage(ctx context.Context, exec boil.ContextExecutor, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpdateAllByPage update all Invitation records from the slice.
// This function updates data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s InvitationSlice) UpdateAllByPage(ctx context.Context, exec boil.ContextExecutor, cols M, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// InsertAllByPage insert all Invitation records from the slice.
// This function inserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s InvitationSlice) InsertAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
	...
	// MySQL max placeholders = 65535
	chunkSize := MaxPageSize / reflect.ValueOf(&InvitationColumns).Elem().NumField()
...
// InsertIgnoreAllByPage insert all Invitation records from the slice.
// This function inserts data by pages to avoid exceeding Postgres limitation (max parameters: 65535)
func (s InvitationSlice) InsertIgnoreAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpsertAllByPage upsert all Invitation records from the slice.
// This function upserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s InvitationSlice) UpsertAllByPage(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
