...
var ClientWhere = struct {
	...
	UserID:      whereHelperstring{field: "`clients`.`user_id`"},
	Name:        whereHelperstring{field: "`clients`.`name`"},
	Type:        whereHelperstring{field: "`clients`.`type`"},
...
var (
	clientAllColumns            = []string{"id", "user_id", "name", "type", "description", "secret_key", "created_at", "updated_at"}
	clientColumnsWithoutDefault = []string{"id", "user_id", "name", "type", "description", "secret_key"}
...
func (clientL) LoadRedirectUris(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClient interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`redirect_uri.client_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
// LoadRedirectUrisByPage performs eager loading of values by page. This is for a 1-M or N-M relationship.
func (s ClientSlice) LoadRedirectUrisByPage(ctx context.Context, e boil.ContextExecutor, mods ...qm.QueryMod) error {
	return s.LoadRedirectUrisByPageEx(ctx, e, DefaultPageSize, mods...)
}
func (s ClientSlice) LoadRedirectUrisByPageEx(ctx context.Context, e boil.ContextExecutor, pageSize int, mods ...qm.QueryMod) error {
	if len(s) == 0 {
		return nil
	}
	for _, chunk := range chunkSlice[*Client](s, pageSize) {
		if err := chunk[0].L.LoadRedirectUris(ctx, e, false, &chunk, queryMods(mods)); err != nil {
			return err
		}
	}
	return nil
...
func (s ClientSlice) GetLoadedRedirectUris() RedirectURISlice {
	result := make(RedirectURISlice, 0, len(s)*2)
	for _, item := range s {
		if item.R == nil || item.R.RedirectUris == nil {
			continue
		}
		result = append(result, item.R.RedirectUris...)
	}
	return result
}
