...
func (o *AuthorizationSession) Upsert(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(authorizationSessionAllColumns, strmangle.SetIntersect(insert, update))
...
var (
	AuthorizationSessionAllColumns            = authorizationSessionAllColumns
	AuthorizationSessionColumnsWithoutDefault = authorizationSessionColumnsWithoutDefault
	AuthorizationSessionColumnsWithDefault    = authorizationSessionColumnsWithDefault
	AuthorizationSessionPrimaryKeyColumns     = authorizationSessionPrimaryKeyColumns
	AuthorizationSessionGeneratedColumns      = authorizationSessionGeneratedColumns
)

// GetID get ID from model object
func (o *AuthorizationSession) GetID() string {
	return o.ID
}

// GetIDs extract IDs from model objects
func (s AuthorizationSessionSlice) GetIDs() []string {
...
func (s AuthorizationSessionSlice) GetIntfIDs() []interface{} {
...
func (s AuthorizationSessionSlice) ToIDMap() map[string]*AuthorizationSession {
	result := make(map[string]*AuthorizationSession, len(s))
	for _, o := range s {
		result[o.ID] = o
	}
	return result
...
func (s AuthorizationSessionSlice) ToUniqueItems() AuthorizationSessionSlice {
	result := make(AuthorizationSessionSlice, 0, len(s))
...
func (s AuthorizationSessionSlice) FindItemByID(id string) *AuthorizationSession {
	for _, o := range s {
		if o.ID == id {
			return o
		}
...
func (s AuthorizationSessionSlice) FindMissingItemIDs(expectedIDs []string) []string {
	if len(s) == 0 {
		return expectedIDs
	}
	result := []string{}
...
func (o AuthorizationSessionSlice) InsertAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		wl, _ := columns.InsertColumnSet(
			...
			authorizationSessionColumnsWithDefault,
			authorizationSessionColumnsWithoutDefault,
			queries.NonZeroDefaultSet(authorizationSessionColumnsWithDefault, row),
		)
		for _, col := range wl {
			wlCols[col] = struct{}{}
		}
	...
	for _, col := range authorizationSessionAllColumns {
	...
	for i, row := range o {
		...
		if i == 0 {
			sql = "INSERT INTO `authorization_sessions` " + "(`" + strings.Join(wl, "`,`") + "`)" + " VALUES "
		}
		sql += strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), len(vals)+1, len(wl))
		if i != len(o)-1 {
			sql += ","
		}
		valMapping, err := queries.BindMapping(authorizationSessionType, authorizationSessionMapping, wl)
		if err != nil {
			return 0, err
		}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to insert all from authorizationSession slice")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by insertall for authorization_sessions")
	}

	if len(authorizationSessionAfterInsertHooks) != 0 {
		for _, obj := range o {
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o AuthorizationSessionSlice) InsertIgnoreAll(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
...
// IMPORTANT: this will calculate the widest columns from all items in the slice, be careful if you want to use default column values
func (o AuthorizationSessionSlice) UpsertAll(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns) (int64, error) {
	if len(o) == 0 {
		return 0, nil
	}
	...
	for _, row := range o {
		nzUniques := queries.NonZeroDefaultSet(mySQLAuthorizationSessionUniqueColumns, row)
		if len(nzUniques) == 0 {
	...
	if !updateColumns.IsNone() && len(update) == 0 {
		return 0, errors.New("models: unable to upsert authorization_sessions, could not build update column list")
	}
	...
	if len(update) == 0 {
		fmt.Fprintf(
			buf,
			"INSERT IGNORE INTO `authorization_sessions`(%s) VALUES %s",
		...
		fmt.Fprintf(
			buf,
			"INSERT INTO `authorization_sessions`(%s) VALUES %s ON DUPLICATE KEY UPDATE ",
			strings.Join(strmangle.IdentQuoteSlice(dialect.LQ, dialect.RQ, insert), ","),
			strmangle.Placeholders(false, len(insert)*len(o), 1, len(insert)),
		)
	...
	query := buf.String()
	valueMapping, err := queries.BindMapping(authorizationSessionType, authorizationSessionMapping, insert)
	if err != nil {
		return 0, err
	}
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to upsert for authorization_sessions")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by upsert for authorization_sessions")
	}

	if len(authorizationSessionAfterUpsertHooks) != 0 {
		for _, obj := range o {
...
// DeleteAllByPage delete all AuthorizationSession records from the slice.
...
func (s AuthorizationSessionSlice) DeleteAllByPage(ctx context.Context, exec boil.ContextExecutor, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpdateAllByPage update all AuthorizationSession records from the slice.
// This function updates data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s AuthorizationSessionSlice) UpdateAllByPage(ctx context.Context, exec boil.ContextExecutor, cols M, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// InsertAllByPage insert all AuthorizationSession records from the slice.
// This function inserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s AuthorizationSessionSlice) InsertAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
	...
	// MySQL max placeholders = 65535
	chunkSize := MaxPageSize / reflect.ValueOf(&AuthorizationSessionColumns).Elem().NumField()
...
// InsertIgnoreAllByPage insert all AuthorizationSession records from the slice.
// This function inserts data by pages to avoid exceeding Postgres limitation (max parameters: 65535)
func (s AuthorizationSessionSlice) InsertIgnoreAllByPage(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
...
// UpsertAllByPage upsert all AuthorizationSession records from the slice.
// This function upserts data by pages to avoid exceeding Mysql limitation (max placeholders: 65535)
// Mysql Error 1390: Prepared statement contains too many placeholders.
func (s AuthorizationSessionSlice) UpsertAllByPage(ctx context.Context, exec boil.ContextExecutor, updateColumns, insertColumns boil.Columns, limits ...int) (int64, error) {
	length := len(s)
	if length == 0 {
		return 0, nil
	}
